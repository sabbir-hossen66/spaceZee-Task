...
export const Navbar = () => {
  ...
  return (
    <nav className="w-full bg-white shadow-sm">
      <div className="container mx-auto flex justify-between items-center py-4 px-6 lg:px-12">
        ...
        <div className="hidden md:flex space-x-5 text-xl">
          {[FaFacebookF, FaTwitter, FaInstagram].map((Icon, index) => (
            <Icon key={index} className="cursor-pointer transion duration-500 ease-in-out hover:text-orange-500" />
          ))}
        </div>

        {/* Mobile Menu Button */}
        <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-2xl transion duration-500 ease-in-out ">
          {isOpen ? <HiX /> : <HiMenu />}
        </button>
      </div>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <ul className="md:hidden bg-white shadow-md text-center space-y-4 py-6 transion duration-500 ease-in-out ">
          {menuItems.map(({ name, path }) => (
